...
@plugin
def file(messages: List[Dict[str, any]], args: Dict) -> List[Dict[str, any]]:
    file_path = (
        os.path.join(args.exec_dir, args.file)
        if args.non_interactive or args.file
        else path_input(None, args.exec_dir)
    )
...
def copy(messages: List[Dict[str, any]], args: Dict) -> List[Dict[str, any]]:
    message_index = get_valid_index(messages, "copy content of", -1)
...
def run_code_block(code_block: Dict, skip_check: bool = False) -> str:
    ...
    try:
        ...
        user_confirm = (
            input(
                f"Code:\n{code}\nExecute (x) or skip (any) {language} block? "
            ).lower()
            if not skip_check
            else "x"
        )
