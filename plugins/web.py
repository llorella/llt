...
def url_fetch(messages: List[Dict[str, any]], args: Dict, index: int = -1) -> List[Dict[str, any]]:
    message_index = get_valid_index(messages, "fetch url from", index) if not args.non_interactive else content_input()  # prompt is any valid verb that precedes the preposition
    html_content = fetch_html(messages[message_index]['content']) 
    tags = find_tags(html_content, tag=args.tags[0])
    content = []
