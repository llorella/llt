...
from utils import list_input, content_input
...
def get_anthropic_completion(
    messages: List[Dict[str, any]], args: Dict
) -> Dict[str, any]:
...
def _whisper(messages: List[Message], args: argparse.Namespace) -> List[Message]:
    # cm:0 - whisper plugin implementation, or anything better for tts. Open an audio stream to the user's default audio device and stream the audio to it.
    # cm:1 - add a stop button to stop the audio stream
    # .... - inputting wh or whisper should trigger the plugin, which is our play button
    # 0:cm - zeroth codemark, a one shot human-computer conversation, embedded in the code, a real time recording of the conversation. Your responsibility is to write to s codemark, veersus saving with a bookmark, and expand codemark with code. Then, include cm annotations for necessary comments, but don't edit originsl cm block comment. After, review your eork, snnd publish codemark 1.
    return messages


@plugin
def whisper(messages: List[Message], args: argparse.Namespace) -> List[Message]:
    import pyaudio
    import wave
    import threading
    import openai
