#!/usr/bin/env python3
"""
project_context.py - LLT tool for project context analysis and manipulation.
...
from utils import (
    Colors,
    get_project_dir, temp_file, input_handler
)
...
@llt()
def include_project_context(messages: List[Dict], context: Dict[str, Any], index: int = -1) -> List[Dict]:
    """
    ...
    elif context.get("non_interactive", False):
        raise ValueError("Project directory must be specified in non-interactive mode.")
    else:
        project_dir = input_handler.get_path_input("Target project directory")
    ...
    globs = [g.strip() for g in context.get("glob", "").split(',')] if context.get("glob") else []
...
@llt()
def git_ls_files(messages: List[Dict], context: Dict[str, Any], index: int = -1) -> List[Dict]:
    """
    Description: List files tracked by Git in the project directory.
    flag: git_ls_files
    short: gls
    param: directory string None      
    param: others bool False          
    param: cached bool True           
    param: ignored bool False         
    param: exclude_standard bool True 
    """
    if context.get("project_dir"):
        project_dir = context["project_dir"]
    elif context.get("non_interactive", False):
    ...
    directory = context.get("directory") or project_dir
    abs_dir = os.path.abspath(directory)

    command = ["ls-files"]
    if context.get("cached", True):
        command.append("-c")
    if context.get("others", False):
        command.append("-o")
    if context.get("ignored", False):
        command.append("-i")
    if context.get("exclude_standard", True):
    ...
    if returncode != 0:
        ...
        summary = f"Git ls-files failed: {stderr}"
        return messages + [Message(role=context.get("role", "user"), content=summary)]

    if stdout:
        file_count = len(stdout.splitlines())
        summary = f"Git ls-files listed {file_count} file(s) in {abs_dir}."
        formatted = f"{summary}\n\n```ls-files\n{stdout}\n```"
        return messages + [Message(role=context.get("role", "user"), content=formatted)]
    else:
        summary = "No files listed by git ls-files with the given options."
        return messages + [Message(role=context.get("role", "user"), content=summary)]


@llt()
def git_status(messages: List[Dict], context: Dict[str, Any], index: int = -1) -> List[Dict]:
    """
    Description: Show the Git status of the project directory.
    flag: git_status
    short: gs
    param: directory string None      # Directory to run git status in
    param: short bool True            # Use short format (-s)
    param: long bool False            # Use long format (overrides short)
    """
    if context.get("project_dir"):
        project_dir = context["project_dir"]
    elif context.get("non_interactive", False):
    ...
    # Access application-wide settings directly from context
    if not context.get("non_interactive", False):
        Colors.print_colored(f"Git status using role: {context.get('role', 'user')}", Colors.BLUE)
        
    # Show execution directories
    Colors.print_colored(f"Execution directory: {context.get('exec_dir', 'not set')}", Colors.BLUE)
    Colors.print_colored(f"Command directory: {context.get('cmd_dir', 'not set')}", Colors.BLUE)

    command = ["status"]
    use_short = context.get("short", True)
    if context.get("long", False):
    ...
    if returncode != 0:
        ...
        # Add error message to chat history using context.role instead of hardcoded role
        return messages + [Message(role=context.get("role", "user"), content=f"Git status failed: {stderr}")]
    elif stdout:
        ...
        # Add output to chat history using context.role
        formatted = f"```git-status\n{stdout}\n```"
        return messages + [Message(role=context.get("role", "user"), content=formatted)]
    else:
        Colors.print_colored("Git status clean or no changes detected.", Colors.GREEN)
        return messages + [Message(role=context.get("role", "user"), content="Git status: working directory clean")]


@llt()
def git_diff(messages: List[Dict], context: Dict[str, Any], index: int = -1) -> List[Dict]:
    """
    Description: Show Git diff for the project directory.
    flag: git_diff
    short: gd
    param: directory string None     
    param: cached bool False         
    param: commit string None       
    param: file string None          
    param: unified int 0        
    """
    if context.get("project_dir"):
        project_dir = context["project_dir"]
    ...
    # Access LLT general settings from context
    if context.get("non_interactive", False):
        Colors.print_colored("Running in non-interactive mode", Colors.BLUE)
    
    Colors.print_colored(f"Language directory: {context.get('ll_dir', 'not set')}", Colors.BLUE)

    command = ["diff"]
    if context.get("cached", False):
        command.append("--cached")
    if context.get("commit"):
        command.append(context["commit"])
    if context.get("unified"):
        try:
            context_lines = int(context["unified"])
    ...
    if returncode != 0:
        ...
        # Add error message using context.role
        return messages + [Message(role=context.get("role", "user"), content=f"Git diff failed: {stderr}")]
    elif stdout:
        Colors.print_colored("--- Git Diff Output ---", Colors.CYAN)
        # Format diff output within a code block for better readability
        diff_output = f"```diff\n{stdout}\n```"
        Colors.print_colored(diff_output, Colors.CYAN)
        # Add the diff output as a message
        return messages + [Message(role=context.get("role", "user"), content=diff_output)]
    else:
        Colors.print_colored("No differences found.", Colors.GREEN)
        return messages + [Message(role=context.get("role", "user"), content="No differences found.")]
